...
Sourced from [ppt-master](https://github.com/hugohe3/ppt-master) by Hugo He. An AI-driven presentation generation system that converts source documents (PDF/DOCX/URL/Markdown) into **natively editable PPTX** with real PowerPoint shapes (DrawingML) through multi-role collaboration (Strategist → Image_Generator → Executor). Unlike image-based PPT tools, every element in the output is directly clickable and editable in PowerPoint.

**Prerequisites**: Python 3.10+ and `pip install -r requirements.txt` (auto-detected on first use).

| Skill | Description |
|-------|-------------|
| **ppt-master** | AI-driven PPT generation: source document → multi-role collaboration → SVG pages → natively editable PPTX. Includes 7-step pipeline, 10 standalone workflows, 30+ Python scripts, brand/layout/chart/icon templates |

**Key features**:
- **Real PowerPoint** — every element is natively editable (not flattened images)
- **Multi-source input** — PDF, DOCX, Excel, PPTX, web URLs, Markdown
- **Multi-role pipeline** — Strategist (8 confirmations) → Image Generator → Executor (SVG) → Post-processing → PPTX export
...
| Skill | Description |
|-------|-------------|
| **taste-skill** | Core anti-slop frontend skill: reads the brief, infers the design direction via three dials (Design Variance / Motion Intensity / Visual Density), and ships interfaces that do not look templated. Real design systems when applicable, audit-first on redesigns, strict pre-flight check |
| **redesign-skill** | Existing project upgrade: audits current design, identifies generic AI patterns, and applies high-end design standards without breaking functionality. Works with any CSS framework or vanilla CSS |
| **output-skill** | Full-output enforcement: overrides default LLM truncation behavior, bans placeholder patterns, and handles token-limit splits cleanly |
| **soft-skill** | High-end visual design: teaches the AI to design like a premium agency — precise fonts, spacing, shadows, card structures, and animations that make a website feel expensive |
| **minimalist-skill** | Minimalist UI: editorial-style interfaces (Notion/Linear vibes) with warm monochrome palette, typographic contrast, flat bento grids, muted pastels. No gradients, no heavy shadows |
| **brutalist-skill** | Industrial brutalist UI: raw mechanical interfaces fusing Swiss typographic print with military terminal aesthetics. Rigid grids, extreme type scale contrast, utilitarian color, analog degradation effects |
...
### Hooks Integration (v5.1 Update)

The following gstack skills now include **hook adaptations** for TRAE IDE's solo mode, enabling automatic enforcement through the hooks mechanism:

- **careful** — PreToolUse hook checks destructive commands before execution
- **freeze** — PreToolUse hook enforces edit boundaries on Write/Edit operations
- **guard** — SessionStart hook injects comprehensive protection rules
- **investigate** — SessionStart hook loads systematic debugging context
...
- [ppt-master](https://github.com/hugohe3/ppt-master) — AI-driven PPT generation (by Hugo He)

## Changelog

### v5.1 — gstack Skills: Hooks Adaptation for TRAE IDE Solo Mode

- Added **hooks integration** for 4 core gstack skills: `careful`, `freeze`, `guard`, `investigate`
- **careful** now enforces destructive command checks via `PreToolUse:RunCommand` hook
- **freeze** now enforces edit boundary checks via `PreToolUse:Edit` and `PreToolUse:Write` hooks
- **guard** now injects comprehensive protection rules via `SessionStart` hook
- **investigate** now loads systematic debugging context via `SessionStart` hook
- Packaged `hooks-config.zip` containing:
  - `hooks.json` — Project-level hook configuration with event matchers and timeouts
  - PowerShell hook scripts: `check-careful.ps1`, `check-freeze.ps1`, `session-start.ps1`
- Updated SKILL.md for all 4 skills with hook adaptation notes and TRAE-specific instructions
- Hooks enable **automatic safety enforcement** without manual skill invocation

### v5.0 — ppt-master: AI-Generated Natively Editable PPTX

- Added [ppt-master](https://github.com/hugohe3/ppt-master) skill — AI-driven presentation generation system
- Converts source documents (PDF/DOCX/URL/Markdown) into **natively editable PPTX** with real PowerPoint shapes
- 7-step pipeline: source processing → project init → template → strategist (8 confirmations) → image acquisition → executor (SVG) → post-processing & export
- 10 standalone workflows: topic research, template fill, create template/brand, resume execute, verify charts, customize animations, live preview, visual review, audio narration
- 30+ Python scripts for source conversion, image generation, SVG processing, and PPTX export
- Template system: brands (Anthropic, Google), layouts (7 types), decks (8 types including Chinese brands), charts (50+ SVG), icons (3 libraries)
- Progressive disclosure architecture: SKILL.md ~345 lines, detailed specs loaded on demand
- Auto-detection of Python environment and dependencies on first use
- Bilingual: Chinese frontmatter/description, English instructions (matching original project)
