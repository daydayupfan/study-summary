...
前端开发相关技能，涵盖设计、模式和演示文稿制作。**v4.1 更新：三个技能均新增强制的"Phase 0: 风格发现与确认"工作流，基于 [Refero Styles](https://styles.refero.design/)。开工前，LLM 会先浏览 Refero 风格库，提取设计令牌（颜色、排版、间距、组件），生成视觉小样供用户比较，仅在用户确认风格方向后才开始实现。**

| 技能名称 | 功能简介 |
|---------|---------|
| **frontend-design** | 前端设计：创建有辨识度、生产级的前端界面，强调视觉方向和设计意图。**新增：强制 Refero 风格发现 → 小样生成 → 用户选择 → 风格锁定后开工** |
| **frontend-patterns** | 前端开发模式：React/Next.js 组件模式、状态管理、性能优化。**新增：涉及视觉呈现时触发风格发现；Refero 令牌到组件的映射规则与角色保持** |
| **frontend-slides** | 前端幻灯片：创建零依赖、动画丰富的 HTML 演示文稿。**新增：双源风格发现（Refero 优先 + 内置预设回退）；支持混合策略** |

## ppt-master（AI生成PPT）

源自 [ppt-master](https://github.com/hugohe3/ppt-master)（Hugo He 出品）。AI驱动的演示文稿生成系统，通过多角色协作（策略师 → 图片生成器 → 执行器）将源文档（PDF/DOCX/URL/Markdown）转换为**原生可编辑的PPTX**，使用真实的PowerPoint形状（DrawingML）。与图片式PPT工具不同，输出中的每个元素都可以在PowerPoint中直接点击编辑。

**前置条件**：Python 3.10+ 及 `pip install -r requirements.txt`（首次使用时自动检测）。

| 技能名称 | 功能简介 |
|---------|---------|
| **ppt-master** | AI驱动PPT生成：源文档 → 多角色协作 → SVG页面 → 原生可编辑PPTX。包含7步流水线、10个独立工作流、30+ Python脚本、品牌/布局/图表/图标模板 |

**核心特性**：
- **真正的PowerPoint** — 每个元素原生可编辑（非扁平化图片）
- **多源输入** — PDF、DOCX、Excel、PPTX、网页URL、Markdown
- **多角色流水线** — 策略师（八确认）→ 图片生成器 → 执行器（SVG）→ 后处理 → PPTX导出
- **模板系统** — 品牌（Anthropic、Google等）、布局（7种）、完整模板（8种含中国品牌）、图表（50+ SVG）、图标（3种库）
- **独立工作流** — 主题研究、模板填充、实时预览、视觉审查、语音旁白、动画自定义等
- **渐进披露架构** — SKILL.md 约345行，详细角色定义和技术规范按需从 `references/` 和 `workflows/` 加载

## taste-skill（反AI俗套前端设计）

源自 [Leonxlnx/taste-skill](https://github.com/Leonxlnx/taste-skill)。一套可移植的 Agent 技能，用于升级 AI 生成的前端界面：更强的布局、排版、动效和间距，告别模板化 UI。这些技能系统性地打破 LLM 生成前端时"看起来都一样"的统计偏见。

| 技能名称 | 功能简介 |
|---------|---------|
| **taste-skill** | 核心反俗套前端技能：读取需求，通过三旋钮（设计方差/动效强度/视觉密度）推断设计方向，输出不像模板的界面。适用时使用真实设计系统，改版时先审计，严格的预飞检查 |
| **redesign-skill** | 已有项目升级：审计现有设计，识别 AI 俗套模式，在不破坏功能的前提下应用高端设计标准。适用于任何 CSS 框架或原生 CSS |
| **output-skill** | 完整输出强制：覆盖默认的 LLM 截断行为，禁止占位符模式，在 token 限制时干净地分割输出 |
| **soft-skill** | 高端视觉设计：教 AI 像高端设计机构一样设计——精确的字体、间距、阴影、卡片结构和动画，让网站感觉昂贵 |
| **minimalist-skill** | 极简 UI：编辑器式界面（Notion/Linear 风格），温暖单色调色板、排版对比、扁平 Bento 网格、柔和粉彩点缀。无渐变、无重阴影 |
| **brutalist-skill** | 工业粗野 UI：融合瑞士排版与军事终端美学的原始机械界面。刚性网格、极端字体比例对比、功利主义配色、模拟退化效果 |
...
以下 gstack 技能现已包含 **Hooks 适配**，可在 TRAE IDE SOLO 模式下通过 hooks 机制自动执行安全防护：

- **careful** — 通过 `PreToolUse:RunCommand` hook 在执行前检查破坏性命令
- **freeze** — 通过 `PreToolUse:Edit` 和 `PreToolUse:Write` hook 强制编辑边界检查
- **guard** — 通过 `SessionStart` hook 注入全面保护规则
- **investigate** — 通过 `SessionStart` hook 加载系统化调试上下文

Hook 配置文件和 PowerShell 脚本打包在 `hooks-config.zip` 中。解压到 `.trae/hooks/` 目录并配置 `.trae/hooks.json` 即可启用自动安全防护。
...
- [ppt-master](https://github.com/hugohe3/ppt-master) — AI驱动PPT生成（Hugo He 出品）

## 更新日志

### v5.1 — gstack 技能：TRAE IDE SOLO 模式 Hooks 适配

- 为 4 个核心 gstack 技能添加 **Hooks 集成**：`careful`、`freeze`、`guard`、`investigate`
- **careful** 通过 `PreToolUse:RunCommand` hook 执行破坏性命令检查
- **freeze** 通过 `PreToolUse:Edit` 和 `PreToolUse:Write` hook 执行编辑边界检查
- **guard** 通过 `SessionStart` hook 注入全面保护规则
- **investigate** 通过 `SessionStart` hook 加载系统化调试上下文
- 打包 `hooks-config.zip`，包含：
  - `hooks.json` — 项目级 hook 配置，含事件匹配器和超时设置
  - PowerShell hook 脚本：`check-careful.ps1`、`check-freeze.ps1`、`session-start.ps1`
- 更新 4 个技能的 SKILL.md，添加 hook 适配说明和 TRAE 专属指令
- Hooks 实现**自动安全防护**，无需手动调用技能

### v5.0 — ppt-master：AI生成原生可编辑PPTX

- 新增 [ppt-master](https://github.com/hugohe3/ppt-master) 技能 — AI驱动的演示文稿生成系统
- 将源文档（PDF/DOCX/URL/Markdown）转换为**原生可编辑PPTX**，使用真实PowerPoint形状
- 7步流水线：源内容处理 → 项目初始化 → 模板 → 策略师八确认 → 图片获取 → 执行器SVG生成 → 后处理与导出
- 10个独立工作流：主题研究、模板填充、创建模板/品牌、恢复执行、图表校验、自定义动画、实时预览、视觉审查、语音旁白
- 30+ Python脚本：源文档转换、图片生成、SVG处理、PPTX导出
- 模板系统：品牌（Anthropic、Google等）、布局（7种）、完整模板（8种含中国品牌）、图表（50+ SVG）、图标（3种库）
- 渐进披露架构：SKILL.md 约345行，详细角色定义和技术规范按需加载
- 首次使用时自动检测Python环境和依赖
- 双语：frontmatter/描述中文，指令英文（与原项目一致）

### v4.1 — 前端技能：Refero 风格发现集成

- 三个前端技能均新增强制的 **Phase 0: 风格发现与确认** 工作流
- 基于 [Refero Styles](https://styles.refero.design/) — 从真实产品页面提取的语义化设计参考
- 工作流：分析需求 → 浏览 Refero 风格库 → 提取设计令牌 → 生成视觉小样 → 用户选择 → 风格锁定 → 开始实现
- **frontend-design**：完整的 6 步 Phase 0，含 UUID 参考表、小样模板、风格锁定格式、Refero MCP 可选增强、回退方案
- **frontend-patterns**：涉及视觉呈现时触发风格发现（纯逻辑任务可跳过）；新增"使用 Refero 令牌实现组件"章节，含令牌映射规则和角色保持
- **frontend-slides**：双源风格发现（Refero 优先 + 内置 STYLE_PRESETS 回退）；支持混合使用 Refero 令牌和预设布局/动效
- 风格锁定机制防止实现偏离用户确认的方向
- 令牌角色保持规则：CTA 色仅用于 CTA、圆角保持指定元素、阴影不滥用等
...
- 工具适配：gstack CLI 引用添加 TRAE 替代方案说明
- 语言统一：所有描述和核心指令中文化
